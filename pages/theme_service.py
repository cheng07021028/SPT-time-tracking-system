...
def apply_theme() -> None:
    ...
    try:
        ...
        pass
    # V2.78: balance white field surface with visible blue breathing glow.
    # Must run after V2.76/V2.77 so it can soften the oversized white surface without losing readable text.
    try:
        apply_v278_field_surface_blue_glow_balance()
    except Exception:
        pass


# ===== V2.78 FIELD SURFACE / BLUE GLOW BALANCE START =====
def apply_v278_field_surface_blue_glow_balance():
    """
    V2.78: reduce the visual dominance of the white field surface and make the cyan breathing glow visible.
    Keeps all dropdown/input text readable: light surface => dark text.
    """
    try:
        import streamlit as st
    except Exception:
    ...
    st.markdown(
        """
        <style id="spt-v278-field-surface-blue-glow-balance">
        /*
          V2.78｜欄位白底與藍色光暈平衡修正
          問題：V2.76/V2.77 為了保證文字可讀，白色輸入面過滿，視覺上把藍框/光暈吃掉。
          原則：淺底仍用深色字，但白底改為淡玻璃面；外層保留藍框呼吸光暈，避免看起來像普通白框。
        */
        :root {
            --spt-v278-field-h: 58px;
            --spt-v278-field-radius: 15px;
            --spt-v278-dark-text: #041827;
            --spt-v278-muted-text: #294962;
            --spt-v278-cyan: #5df0ff;
            --spt-v278-cyan-soft: rgba(93, 240, 255, .42);
            --spt-v278-blue-soft: rgba(76, 118, 255, .18);
        }

        @keyframes sptFieldOuterGlowV278 {
            0%,100% {
                box-shadow:
                    0 0 0 1px rgba(93,240,255,.50),
                    0 0 10px rgba(93,240,255,.16),
                    0 0 24px rgba(76,118,255,.10),
                    inset 0 0 0 1px rgba(255,255,255,.58) !important;
            }
            50% {
                box-shadow:
                    0 0 0 1px rgba(93,240,255,.92),
                    0 0 20px rgba(93,240,255,.38),
                    0 0 44px rgba(76,118,255,.22),
                    inset 0 0 0 1px rgba(255,255,255,.72) !important;
            }
        }

        /* 給欄位本身一點呼吸空間，避免陰影被相鄰區塊裁切 */
        .stSelectbox, .stMultiSelect, .stTextInput, .stPasswordInput, .stNumberInput, .stDateInput, .stTimeInput,
        div[data-testid="stSelectbox"], div[data-testid="stMultiSelect"], div[data-testid="stTextInput"],
        div[data-testid="stPasswordInput"], div[data-testid="stNumberInput"], div[data-testid="stDateInput"], div[data-testid="stTimeInput"] {
            overflow: visible !important;
            padding-top: 2px !important;
            padding-bottom: 10px !important;
        }

        /* Select / Multiselect：外層是藍框與光暈，內層才是淡玻璃白面 */
        div[data-baseweb="select"] {
            min-height: var(--spt-v278-field-h) !important;
            height: var(--spt-v278-field-h) !important;
            border-radius: var(--spt-v278-field-radius) !important;
            border: 1px solid rgba(93,240,255,.78) !important;
            background:
                linear-gradient(135deg, rgba(93,240,255,.18), rgba(255,255,255,.05) 34%, rgba(114,91,255,.08)),
                rgba(231, 247, 255, .72) !important;
            animation: sptFieldOuterGlowV278 3.4s ease-in-out infinite !important;
            overflow: visible !important;
        ...
        div[data-baseweb="select"]::before {
            ...
            background:
                linear-gradient(180deg, rgba(255,255,255,.82), rgba(226,245,255,.62)),
                radial-gradient(circle at 10% 0%, rgba(93,240,255,.18), transparent 36%) !important;
            box-shadow: inset 0 1px 0 rgba(255,255,255,.85), inset 0 -1px 0 rgba(93,240,255,.18) !important;
            z-index: 0 !important;
        }
        ...
        div[data-baseweb="select"]::after {
            ...
            background: linear-gradient(90deg, transparent, rgba(93,240,255,.82), transparent) !important;
            z-index: 1 !important;
            pointer-events: none !important;
        }
        ...
        div[data-baseweb="select"] div[class*="value" i],
        div[data-baseweb="select"] div[class*="placeholder" i],
        div[data-baseweb="select"] div[class*="input" i] {
            min-height: calc(var(--spt-v278-field-h) - 8px) !important;
            height: calc(var(--spt-v278-field-h) - 8px) !important;
            line-height: calc(var(--spt-v278-field-h) - 8px) !important;
            display: flex !important;
            align-items: center !important;
            background: transparent !important;
            color: var(--spt-v278-dark-text) !important;
            -webkit-text-fill-color: var(--spt-v278-dark-text) !important;
            opacity: 1 !important;
            z-index: 2 !important;
            overflow: visible !important;
            box-shadow: none !important;
        ...
        div[data-baseweb="select"] svg {
            color: var(--spt-v278-dark-text) !important;
            -webkit-text-fill-color: var(--spt-v278-dark-text) !important;
            fill: var(--spt-v278-dark-text) !important;
            opacity: 1 !important;
            text-shadow: none !important;
        }

        /* Text / Password / Date / Number：同樣改為淡玻璃白面，不讓白框吃掉光暈 */
        ...
        div[data-testid="stTimeInput"] div[data-baseweb="input"] {
            ...
            height: var(--spt-v278-field-h) !important;
            border-radius: var(--spt-v278-field-radius) !important;
            border: 1px solid rgba(93,240,255,.76) !important;
            background:
                linear-gradient(180deg, rgba(255,255,255,.78), rgba(226,245,255,.66)),
                radial-gradient(circle at 8% 0%, rgba(93,240,255,.16), transparent 38%) !important;
            animation: sptFieldOuterGlowV278 3.4s ease-in-out infinite !important;
            overflow: visible !important;
        ...
        div[data-testid="stTimeInput"] input {
            min-height: calc(var(--spt-v278-field-h) - 2px) !important;
            height: calc(var(--spt-v278-field-h) - 2px) !important;
            line-height: 28px !important;
            color: var(--spt-v278-dark-text) !important;
            -webkit-text-fill-color: var(--spt-v278-dark-text) !important;
            font-weight: 900 !important;
        ...
        div[data-baseweb="base-input"] input::placeholder {
            color: rgba(41,73,98,.72) !important;
            -webkit-text-fill-color: rgba(41,73,98,.72) !important;
            opacity: 1 !important;
        }
        ...
        div[data-baseweb="input"] button {
            ...
            background: linear-gradient(135deg, rgba(118,235,255,.86), rgba(158,185,255,.78)) !important;
            color: var(--spt-v278-dark-text) !important;
            box-shadow: inset 1px 0 0 rgba(255,255,255,.62) !important;
        }

        /* 展開選單維持科技感，但仍是淺底深字，確保不靠 hover 才看得到 */
        div[data-baseweb="popover"],
        div[data-baseweb="menu"],
        ul[role="listbox"] {
            background:
                linear-gradient(180deg, rgba(255,255,255,.98), rgba(228,247,255,.96)) !important;
            border: 1px solid rgba(93,240,255,.86) !important;
            box-shadow: 0 18px 46px rgba(0,8,24,.42), 0 0 34px rgba(93,240,255,.36) !important;
            border-radius: 14px !important;
        ...
        div[role="option"]:hover, li[role="option"]:hover, ul[role="listbox"] li:hover,
        div[role="option"][aria-selected="true"], li[role="option"][aria-selected="true"] {
            background: linear-gradient(90deg, rgba(93,240,255,.90), rgba(205,250,255,.94)) !important;
            color: #020817 !important;
            -webkit-text-fill-color: #020817 !important;
        }

        /* 日期/文字篩選區常常緊貼，額外保留一點底部空隙給光暈 */
        .element-container:has([data-baseweb="select"]),
        .element-container:has(div[data-baseweb="input"]) {
            overflow: visible !important;
...
# ===== V2.78 FIELD SURFACE / BLUE GLOW BALANCE END =====
