...
def parse_pasted_table(text: str) -> pd.DataFrame:
    """Robust parser for Excel copied text.

    Fixes pandas ValueError caused by inconsistent pasted column counts.
    Supports tab-separated Excel paste first, then comma CSV fallback.
    """
    text = (text or "").strip()
    if not text:
        return pd.DataFrame()

    # Excel copy/paste is normally tab separated. csv reader is safer than split().
    ...
    # If first row looks like header, use it. Otherwise create generic columns.
    first = rows[0]
...
with TAB_PASTE:
    ...
    if st.button("解析並匯入貼上資料", use_container_width=True):
