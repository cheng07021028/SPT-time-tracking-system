...
from services.time_record_service import (
    ...
    load_records,
    load_finished_today_for_employee,
...
def _v148_load_today_finished_records_for_employee(employee_id: str, employee_name: str = "") -> pd.DataFrame:
    """V258 fast read-only list for operators to verify today's completed records.

    Uses one SQL-first service query instead of load_records()+today_records() concat.
    This keeps the same read-only panel behavior while avoiding full page rerun cost.
    """
    try:
        df = load_finished_today_for_employee(employee_id, employee_name)
        if isinstance(df, pd.DataFrame):
            return _v148_sort_finished_records(_v148_dedupe_records(df))
    except Exception:
        pass
    return pd.DataFrame()
