...
# V112：系統預設帳號不可停用/刪除，避免 admin 被停用後無法登入。
V112_PROTECTED_DEFAULT_USERS = {"admin", "manager", "leader", "operator", "viewer", "auditor"}

def _v112_is_protected_account(username) -> bool:
    return str(username or "").strip().lower() in V112_PROTECTED_DEFAULT_USERS

def _v112_restore_protected_flags(df: pd.DataFrame) -> pd.DataFrame:
    if not isinstance(df, pd.DataFrame) or df.empty or "帳號 / Username" not in df.columns:
        return df
    out = df.copy()
    mask = out["帳號 / Username"].astype(str).str.strip().str.lower().isin(V112_PROTECTED_DEFAULT_USERS)
    if "啟用 / Active" in out.columns:
        out.loc[mask, "啟用 / Active"] = True
    if "刪除 / Delete" in out.columns:
        out.loc[mask, "刪除 / Delete"] = False
    return out
...
def _users_for_editor() -> pd.DataFrame:
    ...
    return _v112_restore_protected_flags(out)
...
def _account_import_to_editor_rows(import_df: pd.DataFrame) -> pd.DataFrame:
    ...
    return _v112_restore_protected_flags(pd.DataFrame(rows))
...
with tab_accounts:
    ...
    with account_tab_edit:
        ...
        def _v56_prepare_account_draft() -> pd.DataFrame:
            ...
            df0 = _v112_restore_protected_flags(df0)
        ...
        st.info("V112：admin / manager / leader / operator / viewer / auditor 為系統預設帳號，不可停用或刪除；若誤勾，儲存時會自動恢復啟用。")
        ...
        if submitted_accounts:
            ...
            # V112：送出前強制保護系統預設帳號，避免 admin 被停用後整站無法登入。
            attempted_protected_delete = []
            try:
                attempted_protected_delete = [str(x) for x in _selected_delete_usernames(df, account_editor_key) if _v112_is_protected_account(x)]
            except Exception:
                attempted_protected_delete = []
            df = _v112_restore_protected_flags(df)
            # 送出瞬間同步 session 草稿，避免 checkbox delta 尚未併入 dataframe 時漏判。
            st.session_state["v133_users_df"] = df.copy()
            to_delete = [u for u in _selected_delete_usernames(df, account_editor_key) if not _v112_is_protected_account(u)]
            if to_delete:
            ...
            if attempted_protected_delete:
                st.warning("系統預設帳號不可停用/刪除，已自動恢復啟用：" + ", ".join(attempted_protected_delete))
            if to_delete and deleted == 0:
                st.warning("已偵測到刪除勾選，但未刪除任何帳號；系統預設帳號不可刪除，其他帳號請確認帳號欄位是否有效。")
