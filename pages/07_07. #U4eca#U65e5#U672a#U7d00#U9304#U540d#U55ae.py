...
render_header(
    "07｜今日未紀錄名單 / 今日出勤設定",
    "出勤 / 在廠狀態維護、今日未紀錄人員查詢。編輯後需按確認儲存才會永久套用。",
)

STATE_KEY = "v197_today_attendance_editor"
EDITOR_REV_KEY = "v197_today_attendance_editor_rev"
COLS = [
...
tab1, tab2 = st.tabs(["今日出勤設定", "今日未紀錄名單"])
...
with tab1:
    ...
    if not can_edit:
        ...
        render_table(view_df, "today_attendance_readonly", editable=False, height=620)
    else:
        st.info("這裡只維護人員主檔中的『在廠』與『今日出勤』狀態；修改後必須按『確認儲存今日出勤設定』才會永久套用。")

        c1, c2, c3, c4 = st.columns(4)
        if c1.button("🏭 全選在廠", use_container_width=True, key="today_factory_all_on"):
            st.session_state[STATE_KEY]["is_in_factory"] = True
            touch_editor()
            rerun()
        if c2.button("🏭 取消全選在廠", use_container_width=True, key="today_factory_all_off"):
            st.session_state[STATE_KEY]["is_in_factory"] = False
            touch_editor()
            rerun()
        if c3.button("📅 全選今日出勤", use_container_width=True, key="today_attendance_all_on"):
            st.session_state[STATE_KEY]["is_today_attendance"] = True
            touch_editor()
            rerun()
        if c4.button("📅 取消全選今日出勤", use_container_width=True, key="today_attendance_all_off"):
            st.session_state[STATE_KEY]["is_today_attendance"] = False
            touch_editor()
            rerun()

        c5, c6, c7, c8 = st.columns(4)
        if c5.button("✅ 啟用全選", use_container_width=True, key="today_active_all_on"):
            st.session_state[STATE_KEY]["is_active"] = True
            touch_editor()
            rerun()
        if c6.button("⬜ 啟用全取消", use_container_width=True, key="today_active_all_off"):
            st.session_state[STATE_KEY]["is_active"] = False
            touch_editor()
            rerun()
        if c7.button("🔄 重新載入", use_container_width=True, key="today_reload"):
            reload_employees()
            rerun()
        c8.caption("批次按鈕只改畫面暫存，按儲存後才寫入。")

        editor_key = f"today_attendance_editor_v197_{st.session_state.get(EDITOR_REV_KEY, 0)}"
        ...
        if st.button("💾 確認儲存今日出勤設定 / Save Today Attendance", type="primary", use_container_width=True, key="save_today_attendance_v197"):
...
with tab2:
    ...
    render_table(df, "missing_today", editable=False, height=620)
