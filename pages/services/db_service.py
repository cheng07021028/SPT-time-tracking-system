...
# ================= V258 POSTGRESQL SCHEMA + SQLITE-SQL COMPAT HOTFIX｜2026-05-31 =================
# Purpose:
# - Fix repeated PostgreSQL errors found by V257 diagnostics:
#   1) time_records.is_deleted / delete_flag missing while query filters reference them.
#   2) SQLite-only datetime('now','localtime') used against PostgreSQL table_ui_settings.
# - Keep UI/CSS untouched; only backend schema/query compatibility.

try:
    _v258_prev_pg_translate_sql = _v25_pg_translate_sql  # type: ignore[name-defined]
except Exception:  # pragma: no cover
    _v258_prev_pg_translate_sql = None  # type: ignore[assignment]


def _v258_pg_now_expr() -> str:
    return "TO_CHAR(NOW() AT TIME ZONE 'Asia/Taipei', 'YYYY-MM-DD HH24:MI:SS')"


def _v25_pg_translate_sql(sql: str) -> str:  # type: ignore[override]
    raw = str(sql or "")
    # Translate common SQLite time expression before placeholder conversion.
    fixed = raw.replace("datetime('now','localtime')", _v258_pg_now_expr())
    fixed = fixed.replace('datetime("now","localtime")', _v258_pg_now_expr())
    if callable(_v258_prev_pg_translate_sql):
        return _v258_prev_pg_translate_sql(fixed)  # type: ignore[misc]
    return fixed


_V258_PG_SCHEMA_EXT_READY = False


def _v258_pg_column_exists(cur, table_name: str, column_name: str) -> bool:
    try:
        cur.execute(
            """
            SELECT 1
...
def _v258_pg_add_column_if_missing(cur, table_name: str, column_name: str, definition: str) -> None:
    try:
        if not _v258_pg_column_exists(cur, table_name, column_name):
            cur.execute(f"ALTER TABLE {table_name} ADD COLUMN {column_name} {definition}")
    except Exception:
        # Do not block the app; diagnostics will show if the DB user lacks ALTER permission.
        pass


def _v258_pg_ensure_schema_extensions(force: bool = False) -> None:
    global _V258_PG_SCHEMA_EXT_READY
    if _V258_PG_SCHEMA_EXT_READY and not force:
        return
    try:
        if not is_postgres_enabled():  # type: ignore[name-defined]
            _V258_PG_SCHEMA_EXT_READY = True
            return
    except Exception:
        return
    try:
        with _v25_pg_connect() as conn:  # type: ignore[name-defined]
            with conn.cursor() as cur:
                # Deletion flags used by 01/02 filters and large-table SQL helpers.
                _v258_pg_add_column_if_missing(cur, "time_records", "is_deleted", "INTEGER DEFAULT 0")
                _v258_pg_add_column_if_missing(cur, "time_records", "delete_flag", "TEXT DEFAULT ''")
                _v258_pg_add_column_if_missing(cur, "time_records", "deleted_at", "TEXT")
                _v258_pg_add_column_if_missing(cur, "time_records", "deleted_by", "TEXT")
                _v258_pg_add_column_if_missing(cur, "time_records", "delete_reason", "TEXT")

                # Create UI settings table using PostgreSQL-compatible default.
                cur.execute(f"""
                    CREATE TABLE IF NOT EXISTS table_ui_settings (
                        table_key TEXT PRIMARY KEY,
                        widths_json TEXT,
                        order_json TEXT,
                        updated_at TEXT DEFAULT ({_v258_pg_now_expr()})
                    )
                """)
                cur.execute("CREATE INDEX IF NOT EXISTS idx_time_records_v258_active ON time_records(is_deleted, delete_flag, end_timestamp, status, start_date)")
                cur.execute("CREATE INDEX IF NOT EXISTS idx_time_records_v258_emp_finished ON time_records(employee_id, employee_name, start_date, end_timestamp, status, id DESC)")
                cur.execute("CREATE INDEX IF NOT EXISTS idx_time_records_v258_today ON time_records(start_date, id DESC)")
            conn.commit()
        _V258_PG_SCHEMA_EXT_READY = True
    except Exception:
        # Never crash login/app open because of a migration. Query code still has fallbacks.
        pass


try:
    _v258_prev_ensure_database = ensure_database  # type: ignore[name-defined]
except Exception:  # pragma: no cover
    _v258_prev_ensure_database = None  # type: ignore[assignment]


def ensure_database() -> None:  # type: ignore[override]
    if callable(_v258_prev_ensure_database):
        _v258_prev_ensure_database()  # type: ignore[misc]
    _v258_pg_ensure_schema_extensions()


try:
    _v258_prev_get_connection = get_connection  # type: ignore[name-defined]
except Exception:  # pragma: no cover
    _v258_prev_get_connection = None  # type: ignore[assignment]


def get_connection():  # type: ignore[override]
    ensure_database()
    if callable(_v258_prev_get_connection):
        return _v258_prev_get_connection()  # type: ignore[misc]
    if is_postgres_enabled():  # type: ignore[name-defined]
        return _v25_pg_connect()  # type: ignore[name-defined]
    return _open_connection()  # type: ignore[name-defined]


def audit_v258_postgresql_schema_hotfix() -> dict:
    out = {"version": "V258_POSTGRESQL_SCHEMA_SQL_COMPAT_20260531", "postgres_enabled": False, "checked": False}
    try:
        out["postgres_enabled"] = bool(is_postgres_enabled())  # type: ignore[name-defined]
        if not out["postgres_enabled"]:
            return out
        _v258_pg_ensure_schema_extensions(force=True)
        with _v25_pg_connect() as conn:  # type: ignore[name-defined]
            with conn.cursor() as cur:
                out["columns"] = {
                    c: _v258_pg_column_exists(cur, "time_records", c)
                    for c in ["is_deleted", "delete_flag", "deleted_at", "deleted_by", "delete_reason"]
                }
                out["table_ui_settings"] = _v258_pg_column_exists(cur, "table_ui_settings", "table_key")
        out["checked"] = True
    except Exception as exc:
        out["error"] = str(exc)
    return out

# ================= END V258 POSTGRESQL SCHEMA + SQLITE-SQL COMPAT HOTFIX =================
