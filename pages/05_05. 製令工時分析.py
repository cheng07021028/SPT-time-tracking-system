...
df["work_seconds"] = df["work_hours"] * 3600
...
by_wo["total_seconds"] = by_wo["total_hours"] * 3600
...
by_proc["total_seconds"] = by_proc["total_hours"] * 3600
...
by_emp["total_seconds"] = by_emp["total_hours"] * 3600
...
trend["total_seconds"] = trend["total_hours"] * 3600
...
tab1, tab2, tab3, tab4, tab5 = st.tabs(["製令BI", "工段BI", "人員BI", "趨勢BI", "明細編輯"])

with tab1:
    st.subheader("製令累積工時 BI / Work Order Time BI")
    fig = px.bar(
        by_wo.head(30),
        x="work_order",
        y="total_seconds",
        text="工時 / Time",
        hover_data={"total_seconds": False, "工時 / Time": True, "count": True},
        labels={"work_order": "製令 / Work Order", "total_seconds": "累積秒數 / Seconds", "count": "筆數"},
        title="Top 30 製令累積工時 / Top Work Order Time",
    )
    fig.update_traces(textposition="outside")
    st.plotly_chart(style_fig(fig, 460), use_container_width=True)
    render_table(by_wo.drop(columns=["total_seconds", "工時 / Time"]), "analysis_by_work_order", editable=False, height=380)

with tab2:
    st.subheader("工段累積工時 BI / Process Time BI")
    fig = px.bar(
        by_proc,
        x="process_name",
        y="total_seconds",
        text="工時 / Time",
        hover_data={"total_seconds": False, "工時 / Time": True, "count": True},
        labels={"process_name": "工段 / Process", "total_seconds": "累積秒數 / Seconds", "count": "筆數"},
        title="工段累積工時 / Process Time",
    )
    fig.update_traces(textposition="outside")
    st.plotly_chart(style_fig(fig, 460), use_container_width=True)
    render_table(by_proc.drop(columns=["total_seconds", "工時 / Time"]), "analysis_by_process", editable=False, height=380)

with tab3:
    st.subheader("人員累積工時 BI / Employee Time BI")
    fig = px.bar(
        by_emp.head(40),
        x="employee_name",
        y="total_seconds",
        color="employee_id",
        text="工時 / Time",
        hover_data={"total_seconds": False, "工時 / Time": True, "count": True, "employee_id": True},
        labels={"employee_name": "人員 / Employee", "total_seconds": "累積秒數 / Seconds", "employee_id": "工號"},
        title="Top 40 人員累積工時 / Top Employee Time",
    )
    fig.update_traces(textposition="outside")
    st.plotly_chart(style_fig(fig, 480), use_container_width=True)
    render_table(by_emp.drop(columns=["total_seconds", "工時 / Time"]), "analysis_by_employee", editable=False, height=380)

with tab4:
    st.subheader("每日趨勢 BI / Daily Trend BI")
    fig = px.line(
        trend,
        x="start_date",
        y="total_seconds",
        ...
        text="工時 / Time",
        hover_data={"total_seconds": False, "工時 / Time": True, "count": True},
        labels={"start_date": "日期 / Date", "total_seconds": "累積秒數 / Seconds"},
        title="每日累積工時趨勢 / Daily Time Trend",
    )
    fig.update_traces(textposition="top center")
    st.plotly_chart(style_fig(fig, 430), use_container_width=True)
    render_table(trend.drop(columns=["total_seconds", "工時 / Time"]), "analysis_daily_trend", editable=False, height=320)

with tab5:
    st.caption("此處編輯的是分析來源明細，儲存後會影響歷史紀錄與後續統計。工時欄位以 00:00:00 顯示，需調整時請改開始/結束時間後重新計算。")
    edited = render_table(df.drop(columns=["work_seconds"], errors="ignore"), "analysis_detail_records", editable=True, disabled=["id", "record_key", "created_at", "updated_at", "work_hours"], key="analysis_detail_editor", height=520)
