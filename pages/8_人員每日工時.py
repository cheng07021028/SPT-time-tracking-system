...
selected = st.date_input("日期", value=date.today())
...
if not df.empty:
    ...
    c1.metric("出勤在廠人數", f"{len(df):,}")
    c2.metric("未紀錄", f"{(df['status']=='未紀錄').sum():,}")
    c3.metric("偏低", f"{(df['status']=='偏低').sum():,}")
    c4.metric("正常/超時", f"{((df['status']=='正常') | (df['status']=='超時')).sum():,}")
