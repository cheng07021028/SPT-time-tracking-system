...
def touch_editor() -> None:
    # V57：批次按鈕後清掉舊 data_editor widget state 並換 key，避免畫面又被舊狀態蓋回。
    try:
        for _k0 in list(st.session_state.keys()):
            if str(_k0).startswith("today_attendance_editor_v202_"):
...
if not can_edit:
    ...
    _draft_today = ensure_cols(st.session_state[STATE_KEY].copy())
    for _col in ["is_active", "is_in_factory", "is_today_attendance"]:
        if _col in _draft_today.columns:
            _draft_today[_col] = _draft_today[_col].fillna(False).astype(bool)
    st.session_state[STATE_KEY] = _draft_today
