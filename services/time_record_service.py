...
# ===================== V166D DURABLE LAYER LOG SNAPSHOT MIRROR =====================
# 目的：V166C 只能在一般 write_log() 當下，依 target_id 回查 rows 生成快照；若該 LOG
# 沒有 target_id 或回查時機不完整，快照覆蓋率仍可能不足。V166D 在 V152 durable layer
# 已經取得完整 rows 後，再額外寫入一筆含完整 JSON 快照的 06 LOG。此段只新增稽核 LOG，
# 不改 01/02 正式交易結果、不刪除、不重新編號、不切換資料庫。
try:
    _v166d_prev_v152_write_durable_layers = _v152_write_durable_layers  # type: ignore[name-defined]
except Exception:  # pragma: no cover
    _v166d_prev_v152_write_durable_layers = None


def _v166d_row_count(rows) -> int:
    try:
        if isinstance(rows, pd.DataFrame):
            return int(len(rows))
    except Exception:
...
def _v152_write_durable_layers(rows, reason: str, *, event_type: str = "TIME_RECORD_CHANGE", github: bool = False, extra: dict | None = None) -> int:  # type: ignore[override]
    if callable(_v166d_prev_v152_write_durable_layers):
        n = int(_v166d_prev_v152_write_durable_layers(rows, reason, event_type=event_type, github=github, extra=extra) or 0)
    else:
        n = 0
    try:
        rc = _v166d_row_count(rows)
        if rc > 0:
            from services.log_snapshot_service import write_explicit_time_record_snapshot_log  # type: ignore
            # target_id 儘量帶第一筆 id；還原時仍以 record_key / business key 為準。
            target_id = ""
            try:
                if isinstance(rows, pd.DataFrame) and not rows.empty and "id" in rows.columns:
                    target_id = str(rows.iloc[0].get("id") or "")
                elif isinstance(rows, list) and rows and isinstance(rows[0], dict):
                    target_id = str(rows[0].get("id") or "")
                elif isinstance(rows, dict):
                    target_id = str(rows.get("id") or "")
            except Exception:
                target_id = ""
            write_explicit_time_record_snapshot_log(
                rows,
                action_type="V166D_DURABLE_SNAPSHOT",
                message=f"V166D durable snapshot：{event_type} / {reason} / rows={rc}",
                target_id=target_id,
                reason=reason,
                extra={"event_type": event_type, "github": bool(github), "extra": extra or {}},
                level="INFO",
            )
    except Exception as exc:
        try:
            write_log("V166D_LOG_SNAPSHOT_MIRROR_ERROR", f"V166D durable layer LOG 快照鏡像失敗：{exc}", "time_records", level="WARN")
        except Exception:
            pass
    return n
# =================== END V166D DURABLE LAYER LOG SNAPSHOT MIRROR ===================


# ======================= V166E EXACT DUPLICATE TIME ROW GUARD =======================
try:
    _v166e_prev_save_time_records = save_time_records
except Exception:  # pragma: no cover
    _v166e_prev_save_time_records = None


def save_time_records(df: pd.DataFrame, recalc_edited_timestamps: bool = False) -> int:  # type: ignore[override]
    """V166E: remove only exact duplicate rows before delegating to current save logic.

    This is intentionally conservative: it does not merge rows that have the same
    record_key but different status/end time, so it will not erase legitimate
    close/history variants.  It only prevents the observed triple-identical-row
    symptom from being persisted again.
    """
    from services.duplicate_guard_service import dedupe_time_records_exact
    clean = dedupe_time_records_exact(df)
    if _v166e_prev_save_time_records is not None:
        return int(_v166e_prev_save_time_records(clean, recalc_edited_timestamps=recalc_edited_timestamps))
    return int(len(clean))
# ===================== END V166E EXACT DUPLICATE TIME ROW GUARD =====================
