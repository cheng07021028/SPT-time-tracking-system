...
# =================== V173 01 SQL PRECISE QUERY LAYER ===================
# Purpose:
# - Speed up 01｜工時紀錄 interactive reruns without changing visual rendering.
# - Read only the current employee/date data for live 01 use cases when safe.
# - Keep existing heavy reconciliation path as fallback for full-history/admin calls.
# Safety:
# - No CSS/theme/page layout changes.
# - No write-path changes; start/finish/save/delete/import remain unchanged.
# - No ID renumbering, no authority overwrite, no deletion.
# - SQL precise reads are used only for filtered/current-employee reads.

try:
    _v173_prev_load_records = load_records
except Exception:  # pragma: no cover
    _v173_prev_load_records = None
try:
    _v173_prev_today_records = today_records
except Exception:  # pragma: no cover
    _v173_prev_today_records = None
try:
    _v173_prev_get_active_records = get_active_records
except Exception:  # pragma: no cover
    _v173_prev_get_active_records = None
try:
    _v173_prev_get_active_record = get_active_record
except Exception:  # pragma: no cover
    _v173_prev_get_active_record = None
try:
    _v173_prev_get_active_group = get_active_group
except Exception:  # pragma: no cover
    _v173_prev_get_active_group = None

_V173_TERMINAL_STATUS = {"下班", "暫停", "完工", "已結束", "結束", "停止", "補登結束", "closed", "finished", "complete", "completed", "pause", "paused", "off duty"}
_V173_ACTIVE_STATUS = {"", "作業中", "working", "active", "in progress"}
_V173_ACTIVE_SQL_STATUS = tuple(s for s in _V173_ACTIVE_STATUS if s)
_V173_CACHE_TTL_SECONDS = 6.0
_v173_read_cache: dict[tuple, tuple[float, pd.DataFrame]] = {}


def _v173_time_monotonic() -> float:
    try:
        import time as _time
...
def _v173_cache_get(key: tuple) -> pd.DataFrame | None:
    try:
        hit = _v173_read_cache.get(key)
        if not hit:
            return None
        ts, df = hit
        if _v173_time_monotonic() - ts > _V173_CACHE_TTL_SECONDS:
            _v173_read_cache.pop(key, None)
            return None
        return df.copy()
    except Exception:
        return None
...
def _v173_cache_set(key: tuple, df: pd.DataFrame) -> pd.DataFrame:
    try:
        _v173_read_cache[key] = (_v173_time_monotonic(), df.copy() if isinstance(df, pd.DataFrame) else pd.DataFrame())
    except Exception:
        pass
    return df


def clear_v173_01_precise_query_cache() -> None:
    """Clear only V173 read cache. Called by write wrappers and available to tools."""
    try:
        _v173_read_cache.clear()
    except Exception:
        pass


# Chain existing cache clearer without removing prior behavior.
try:
    _v173_prev_clear_today_records_fast_cache = clear_today_records_fast_cache
except Exception:  # pragma: no cover
    _v173_prev_clear_today_records_fast_cache = None


def clear_today_records_fast_cache() -> None:  # type: ignore[override]
    try:
        if callable(_v173_prev_clear_today_records_fast_cache):
            _v173_prev_clear_today_records_fast_cache()
    except Exception:
        pass
    clear_v173_01_precise_query_cache()


def _v173_norm(value) -> str:
    try:
        if pd.isna(value):
            return ""
    except Exception:
...
def _v173_is_admin_session() -> bool:
    try:
        import streamlit as _st
        roles = _st.session_state.get("auth_roles", []) or []
        username = str(_st.session_state.get("auth_username", "") or "").strip().lower()
        if username == "admin":
            return True
...
def _v173_current_session_employee_id() -> str:
    """Return current logged-in employee for non-admin 01 reads; empty means do not infer."""
    if _v173_is_admin_session():
        return ""
    try:
        import streamlit as _st
        for key in ["auth_employee_id", "employee_id", "current_employee_id"]:
            v = _v173_norm(_st.session_state.get(key))
            if v:
                return v.split(" ", 1)[0].split("｜", 1)[0].strip()
        # Fallback: parse 01 selectbox labels like "SPT001｜王小明".
        for key in ["start_emp_v127", "end_emp_v127", "start_emp", "end_emp"]:
            v = _v173_norm(_st.session_state.get(key))
            if v:
                return v.split("｜", 1)[0].split("/", 1)[0].strip()
    except Exception:
...
def _v173_select_time_records_sql(
    *,
    start_date: str | None = None,
    ...
    include_current_cycle: bool = False,
    order_desc: bool = True,
    limit: int | None = None,
) -> pd.DataFrame:
    """Small, indexed SQL reader for 01 current-person queries."""
    where: list[str] = []
    params: list[object] = []
    if employee_id:
    ...
    if process_name:
        where.append("COALESCE(process_name,'')=?")
        params.append(str(process_name).strip())
    if start_date:
        where.append("COALESCE(start_date, substr(COALESCE(start_timestamp,''),1,10))>=?")
        params.append(str(start_date)[:10])
    if end_date:
        where.append("COALESCE(start_date, substr(COALESCE(start_timestamp,''),1,10))<=?")
        params.append(str(end_date)[:10])
    if active_only:
        where.append("(end_timestamp IS NULL OR trim(COALESCE(end_timestamp,''))='' OR lower(trim(COALESCE(end_timestamp,''))) IN ('none','nan','nat','null'))")
        where.append("(status IS NULL OR trim(COALESCE(status,''))='' OR trim(COALESCE(status,''))='作業中' OR lower(trim(COALESCE(status,''))) IN ('working','active','in progress'))")
    elif include_current_cycle:
        # Show current-cycle rows plus unfinished rows. This mirrors 01 display rules without scanning all history.
        cycle_start = start_date or (_business_cycle_start_date() if "_business_cycle_start_date" in globals() else today_text())
        where.append("((COALESCE(start_date, substr(COALESCE(start_timestamp,''),1,10))>=?) OR (end_timestamp IS NULL OR trim(COALESCE(end_timestamp,''))='' OR lower(trim(COALESCE(end_timestamp,''))) IN ('none','nan','nat','null'))) ")
        params.append(str(cycle_start)[:10])
    sql = "SELECT * FROM time_records"
    if where:
        sql += " WHERE " + " AND ".join(where)
    sql += " ORDER BY id " + ("DESC" if order_desc else "ASC")
...
def _v173_post_filter_active(df: pd.DataFrame, employee_id: str | None = None, employee_name: str | None = None) -> pd.DataFrame:
    if not isinstance(df, pd.DataFrame) or df.empty:
        return pd.DataFrame()
    out = df.copy()
    try:
        if "status" in out.columns:
            s = out["status"].fillna("").astype(str).str.strip()
            out = out.loc[s.map(lambda x: x == "" or x == "作業中" or x.lower() in {"working", "active", "in progress"})].copy()
        if "end_timestamp" in out.columns:
            e = out["end_timestamp"].fillna("").astype(str).str.strip().str.lower()
            out = out.loc[e.isin(["", "none", "nan", "nat", "null"])].copy()
        if employee_id and "employee_id" in out.columns:
            out = out.loc[out["employee_id"].fillna("").astype(str).str.strip() == str(employee_id).strip()].copy()
        if employee_name and "employee_name" in out.columns:
            # Name filter is helpful but not mandatory for older rows; only enforce if the column has a nonblank value.
            name = out["employee_name"].fillna("").astype(str).str.strip()
            out = out.loc[(name == "") | (name == str(employee_name).strip())].copy()
    except Exception:
        pass
    return out.reset_index(drop=True)
...
def get_active_records(employee_id: str | None = None, process_name: str | None = None, start_date: str | None = None, employee_name: str | None = None) -> pd.DataFrame:  # type: ignore[override]
    emp = _v173_norm(employee_id)
    # Safe precise path only when employee is known. Calls without employee keep legacy cross-source guard.
    if not emp:
        if callable(_v173_prev_get_active_records):
            return _v173_prev_get_active_records(employee_id=employee_id, process_name=process_name, start_date=start_date, employee_name=employee_name)
        return pd.DataFrame()
    key = ("v173_active", emp, _v173_norm(employee_name), _v173_norm(process_name), _v173_norm(start_date))
    cached = _v173_cache_get(key)
    if cached is not None:
        return cached
    try:
        df = _v173_select_time_records_sql(employee_id=emp, employee_name=employee_name or None, process_name=process_name or None, start_date=start_date or None, active_only=True, order_desc=False, limit=500)
        df = _v173_post_filter_active(df, emp, employee_name)
        return _v173_cache_set(key, df)
    except Exception:
        if callable(_v173_prev_get_active_records):
            return _v173_prev_get_active_records(employee_id=employee_id, process_name=process_name, start_date=start_date, employee_name=employee_name)
        return pd.DataFrame()
...
def get_active_record(employee_id: str, employee_name: str | None = None) -> dict | None:  # type: ignore[override]
    ...
    try:
        if "id" in df.columns:
            df = df.assign(_v173_id=pd.to_numeric(df["id"], errors="coerce")).sort_values("_v173_id", ascending=False, kind="stable").drop(columns=["_v173_id"], errors="ignore")
        return df.iloc[0].where(pd.notna(df.iloc[0]), None).to_dict()
    except Exception:
        if callable(_v173_prev_get_active_record):
            try:
                return _v173_prev_get_active_record(employee_id, employee_name=employee_name)
            except TypeError:
                return _v173_prev_get_active_record(employee_id)
        return None
...
def get_active_group(record_id: int) -> pd.DataFrame:  # type: ignore[override]
    ...
    if cached is not None:
        return cached
    try:
        base = query_df("SELECT * FROM time_records WHERE id=?", (rid,))
        base = _v173_post_filter_active(base)
        if base.empty:
            return pd.DataFrame()
        row = base.iloc[0].to_dict()
        emp = _v173_norm(row.get("employee_id"))
        emp_name = _v173_norm(row.get("employee_name"))
        proc = _v173_norm(row.get("process_name"))
        sdate = _v173_norm(row.get("start_date") or _v173_norm(row.get("start_timestamp"))[:10])
        if not emp:
            raise ValueError("missing employee_id for active group")
        df = get_active_records(employee_id=emp, employee_name=emp_name or None, process_name=proc or None, start_date=sdate or None)
        return _v173_cache_set(key, df)
    except Exception:
        if callable(_v173_prev_get_active_group):
            return _v173_prev_get_active_group(record_id)
        return pd.DataFrame()


def get_active_same_work(employee_id: str, work_order: str, process_name: str, start_date: str | None = None, employee_name: str | None = None) -> dict | None:  # type: ignore[override]
    df = get_active_records(employee_id=employee_id, employee_name=employee_name, process_name=process_name, start_date=start_date or today_text())
    if df.empty:
        return None
    try:
        target = str(work_order or "").strip()
        if "work_order" in df.columns:
            df = df.loc[df["work_order"].fillna("").astype(str).str.strip() == target].copy()
        if df.empty:
            return None
        return df.iloc[-1].where(pd.notna(df.iloc[-1]), None).to_dict()
    except Exception:
...
def get_conflicting_active_records(employee_id: str, process_name: str, start_date: str | None = None, employee_name: str | None = None) -> pd.DataFrame:  # type: ignore[override]
    ...
    sdate = str(start_date or today_text())[:10]
    try:
        p = active["process_name"].fillna("").astype(str).str.strip() if "process_name" in active.columns else pd.Series([""] * len(active), index=active.index)
        d = active["start_date"].fillna("").astype(str).str[:10] if "start_date" in active.columns else pd.Series([""] * len(active), index=active.index)
        return active.loc[(p != proc) | (d != sdate)].reset_index(drop=True)
    except Exception:
        return active


def load_records(start_date: str | None = None, end_date: str | None = None, employee_id: str | None = None, work_order: str | None = None) -> pd.DataFrame:  # type: ignore[override]
    # For filtered reads, use indexed SQL. Full unfiltered reads keep legacy merge/event behavior.
    filtered = any([start_date, end_date, employee_id, work_order])
    if not filtered:
        if callable(_v173_prev_load_records):
            return _v173_prev_load_records(start_date, end_date, employee_id, work_order)
        return pd.DataFrame()
    key = ("v173_load", _v173_norm(start_date), _v173_norm(end_date), _v173_norm(employee_id), _v173_norm(work_order))
    cached = _v173_cache_get(key)
    if cached is not None:
        return cached
    try:
        df = _v173_select_time_records_sql(start_date=start_date, end_date=end_date, employee_id=employee_id, work_order=work_order, order_desc=True, limit=None)
        return _v173_cache_set(key, df)
    except Exception:
        if callable(_v173_prev_load_records):
            return _v173_prev_load_records(start_date, end_date, employee_id, work_order)
...
def today_records_for_employee(employee_id: str, employee_name: str | None = None, include_finished: bool = True, unfinished_only: bool = False) -> pd.DataFrame:
    emp = _v173_norm(employee_id)
    if not emp:
        return today_records(include_finished=include_finished, unfinished_only=unfinished_only)
    cycle_start = _business_cycle_start_date() if "_business_cycle_start_date" in globals() else today_text()
    key = ("v173_today_emp", emp, _v173_norm(employee_name), bool(include_finished), bool(unfinished_only), cycle_start)
    cached = _v173_cache_get(key)
    if cached is not None:
        return cached
    try:
        df = _v173_select_time_records_sql(employee_id=emp, employee_name=employee_name or None, active_only=bool(unfinished_only), include_current_cycle=not bool(unfinished_only), start_date=cycle_start if not unfinished_only else None, order_desc=True, limit=1000)
        if not include_finished:
            df = _v173_post_filter_active(df, emp, employee_name)
        return _v173_cache_set(key, df.reset_index(drop=True) if isinstance(df, pd.DataFrame) else pd.DataFrame())
    except Exception:
        if callable(_v173_prev_today_records):
            base = _v173_prev_today_records(include_finished=include_finished, unfinished_only=unfinished_only)
            try:
                if isinstance(base, pd.DataFrame) and not base.empty and "employee_id" in base.columns:
                    base = base.loc[base["employee_id"].fillna("").astype(str).str.strip() == emp].copy()
            except Exception:
                pass
            return base if isinstance(base, pd.DataFrame) else pd.DataFrame()
        return pd.DataFrame()
...
def today_records(include_finished: bool = True, unfinished_only: bool = False) -> pd.DataFrame:  # type: ignore[override]
    # Non-admin operator 01 view should never scan all records. Admin/global reports keep legacy behavior.
    emp = _v173_current_session_employee_id()
    if emp:
        return today_records_for_employee(emp, include_finished=include_finished, unfinished_only=unfinished_only)
    if callable(_v173_prev_today_records):
        return _v173_prev_today_records(include_finished=include_finished, unfinished_only=unfinished_only)
    return pd.DataFrame()


def _v173_clear_after_write_wrapper(fn):
...
# Clear precise read cache after write operations. This preserves existing write functions.
for _v173_name in ["start_work", "finish_work", "save_time_records", "delete_time_records", "recalculate_time_records", "import_time_records"]:
    try:
        _v173_fn = globals().get(_v173_name)
        if callable(_v173_fn) and not getattr(_v173_fn, "_v173_cache_wrapped", False):
            _v173_wrapped = _v173_clear_after_write_wrapper(_v173_fn)
            setattr(_v173_wrapped, "_v173_cache_wrapped", True)
            globals()[_v173_name] = _v173_wrapped
    except Exception:
        pass


def get_v173_01_query_optimization_status() -> dict:
    return {
        "version": "V173",
        "enabled": True,
        "visual_changed": False,
        "write_path_changed": False,
        "cache_entries": len(_v173_read_cache),
        "cache_ttl_seconds": _V173_CACHE_TTL_SECONDS,
        "mode": "employee-scoped SQL reads for 01 live data; legacy fallback for admin/full reads",
    }
