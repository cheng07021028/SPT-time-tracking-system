...
MODULE_ROOT = PROJECT_ROOT / "data" / "persistent_modules"
...
def upload_all_permanent_files_to_github(archive: bool = True, source: str = "manual") -> dict[str, Any]:
    ...
    if LATEST_STATE.exists():
        uploads.append(_upload_file(LATEST_STATE, "data/persistent_state/spt_permanent_state.json", f"SPT auto sync latest state {stamp}"))
        if archive:
            uploads.append(_upload_file(LATEST_STATE, f"data/persistent_state/history/spt_permanent_state_{stamp}.json", f"SPT auto sync state history {stamp}"))
    if LATEST_SETTINGS.exists():
        uploads.append(_upload_file(LATEST_SETTINGS, "data/persistent_state/spt_module_settings.json", f"SPT auto sync latest settings {stamp}"))
        if archive:
            uploads.append(_upload_file(LATEST_SETTINGS, f"data/persistent_state/history/spt_module_settings_{stamp}.json", f"SPT auto sync settings history {stamp}"))
    ...
    for module_key in MODULE_TABLES:
        ...
        set_path = module_dir / f"{module_key}_settings.json"
        if rec_path.exists():
            uploads.append(_upload_file(rec_path, f"data/persistent_modules/{module_key}/{module_key}_records.json", f"SPT auto sync {module_key} records {stamp}"))
            if archive:
                uploads.append(_upload_file(rec_path, f"data/persistent_modules/{module_key}/history/{module_key}_records_{stamp}.json", f"SPT auto sync {module_key} records history {stamp}"))
        if set_path.exists():
            uploads.append(_upload_file(set_path, f"data/persistent_modules/{module_key}/{module_key}_settings.json", f"SPT auto sync {module_key} settings {stamp}"))
            if archive:
                uploads.append(_upload_file(set_path, f"data/persistent_modules/{module_key}/history/{module_key}_settings_{stamp}.json", f"SPT auto sync {module_key} settings history {stamp}"))
