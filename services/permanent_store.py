...
SCHEMA_VERSION = "neon-single-source-v31"
...
def ensure_store() -> None:
    # Local folders are compatibility/cache only. Neon is the authority when enabled.
    ...
    if not manifest.exists():
        atomic_write_json(manifest, {
            ...
            "created_at": now_str(),
            "official_root": "Neon/PostgreSQL when DATABASE_URL is configured; local files are fallback/cache only.",
        }, backup=False)
    if _neon_enabled():
        try:
...
def load_records(module_key: str, default_rows: list[dict[str, Any]] | None = None) -> list[dict[str, Any]]:
    if _neon_enabled():
        try:
            from services.neon_authority_service import load_payload, save_payload
            payload = load_payload(str(module_key), "records", None)
            if payload is None:
                rows = list(default_rows or [])
                save_payload(str(module_key), "records", {"schema_version": SCHEMA_VERSION, "created_at": now_str(), "updated_at": now_str(), "rows": rows})
                return rows
...
def save_records(module_key: str, rows: list[dict[str, Any]], user: str = "SYSTEM", action: str = "save_records") -> None:
    if _neon_enabled():
        from services.neon_authority_service import load_payload, save_payload, append_audit
        old = load_payload(str(module_key), "records", {}) or {}
        created_at = old.get("created_at") if isinstance(old, dict) else now_str()
        save_payload(str(module_key), "records", {"schema_version": SCHEMA_VERSION, "created_at": created_at or now_str(), "updated_at": now_str(), "updated_by": user, "rows": rows}, user=user)
        append_audit(str(module_key), action, user, "OK", f"saved {len(rows)} rows")
        return
    path = module_records_path(module_key)
...
def load_settings(module_key: str, default_settings: dict[str, Any] | None = None) -> dict[str, Any]:
    if _neon_enabled():
        try:
            from services.neon_authority_service import load_payload, save_payload
            payload = load_payload(str(module_key), "settings", None)
            if payload is None:
                settings = dict(default_settings or {})
                save_payload(str(module_key), "settings", {"schema_version": SCHEMA_VERSION, "created_at": now_str(), "updated_at": now_str(), "settings": settings})
                return settings
...
def save_settings(module_key: str, settings: dict[str, Any], user: str = "SYSTEM", action: str = "save_settings") -> None:
    if _neon_enabled():
        from services.neon_authority_service import load_payload, save_payload, append_audit
        old = load_payload(str(module_key), "settings", {}) or {}
        created_at = old.get("created_at") if isinstance(old, dict) else now_str()
        save_payload(str(module_key), "settings", {"schema_version": SCHEMA_VERSION, "created_at": created_at or now_str(), "updated_at": now_str(), "updated_by": user, "settings": settings}, user=user)
        append_audit(str(module_key), action, user, "OK", "settings saved")
        return
    path = module_settings_path(module_key)
    old = read_json(path, {}) if path.exists() else {}
    created_at = old.get("created_at") if isinstance(old, dict) else now_str()
...
def load_system(name: str, default: Any = None) -> Any:
    if _neon_enabled():
        try:
            from services.neon_authority_service import load_system_payload, save_system_payload
            payload = load_system_payload(str(name), None)
            if payload is None:
                save_system_payload(str(name), default)
                return default
            return payload
        except Exception:
            pass
...
def save_system(name: str, data: Any) -> None:
    if _neon_enabled():
        ...
        save_system_payload(str(name), data)
        return
...
def log_event(module_key: str, action: str, user: str, result: str = "OK", message: str = "") -> None:
    ...
    if _neon_enabled():
        try:
            from services.neon_authority_service import append_audit
            append_audit(str(module_key), str(action), str(user), str(result), str(message), row)
            return
        except Exception:
            pass
...
def store_health() -> dict[str, Any]:
        ...
        try:
            ...
            for r in status.get("rows", []):
                modules.append({
                    "模組": r.get("module_key"),
                    "種類": r.get("kind"),
                    "Neon資料大小Bytes": r.get("payload_bytes", 0),
                    "最後更新": r.get("updated_at", ""),
                    "來源": "Neon/PostgreSQL",
                })
            return {"root": "neon://spt_module_authority", "modules": modules, "backend": "neon"}
        except Exception as exc:
            return {"root": "neon://spt_module_authority", "modules": [], "backend": "neon", "error": str(exc)}
    modules = []
    for p in sorted(MODULE_ROOT.iterdir()) if MODULE_ROOT.exists() else []:
        if not p.is_dir():
            continue
        rec = p / "records.json"
    ...
    return {"root": str(STORE_ROOT.relative_to(PROJECT_ROOT)), "modules": modules, "backend": "local"}
