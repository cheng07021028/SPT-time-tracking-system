...
# ===== V255 APP STARTUP / LOGIN HOT PATH FOR POSTGRESQL =====
# Problem fixed:
# - Opening the app/login page should not initialize the full 01~13 business schema.
# - Auth/security SQL should only prepare auth/security tables, then reuse the V29 PG connection.
# - Business pages still use the normal full ensure_database() path.

_V255_PG_AUTH_SCHEMA_READY = False


def _v255_is_auth_sql(sql: str | None) -> bool:
    try:
        return _is_auth_or_security_sql(sql or "") and not _is_business_sql(sql or "")
    except Exception:
        text = (sql or "").lower()
        return any(t in text for t in (
            "auth_users", "auth_account_permissions", "auth_login_logs", "auth_security_settings",
            "security_users", "security_roles", "security_user_roles", "security_module_permissions",
            "security_settings", "security_login_logs",
        ))


def _v255_pbkdf2_hash(password: str) -> str:
    import base64 as _b64
    import hashlib as _hashlib
    import os as _os
    salt = _os.urandom(16)
    dk = _hashlib.pbkdf2_hmac("sha256", str(password).encode("utf-8"), salt, 180000)
    return "pbkdf2_sha256${}${}${}".format(
        180000,
        _b64.b64encode(salt).decode("ascii"),
        _b64.b64encode(dk).decode("ascii"),
    )


def _v255_pg_ensure_auth_schema() -> None:
    global _V255_PG_AUTH_SCHEMA_READY
    if _V255_PG_AUTH_SCHEMA_READY:
        return
    if not is_postgres_enabled():
        return
    now = _now()
    ...
    modules = [
        ("01_time_record", "01", "工時紀錄", "Time Record"),
        ("02_history", "02", "歷史紀錄", "History"),
        ("03_work_orders", "03", "製令管理", "Work Orders"),
        ("04_employees", "04", "人員名單", "Employees"),
        ("05_analysis", "05", "製令工時分析", "Analysis"),
        ("06_logs", "06", "LOG查詢", "Logs"),
        ("07_missing", "07", "今日未紀錄名單", "Missing Today"),
        ("08_daily_hours", "08", "人員每日工時", "Daily Hours"),
        ("09_persistence", "09", "資料永久保存與備份", "Persistence"),
        ("10_permissions", "10", "權限管理", "Permissions"),
        ("11_login_logs", "11", "登入紀錄", "Login Logs"),
        ("12_module_persistence", "12", "模組永久紀錄中心", "Module Permanent Records"),
        ("13_system_settings", "13", "系統設定", "System Settings"),
    ]
    conn = _v29_pg_connection() if "_v29_pg_connection" in globals() else _v25_pg_connect()
    with conn.cursor() as cur:
        for stmt in auth_tables:
            cur.execute(stmt)
        cur.execute("CREATE INDEX IF NOT EXISTS idx_auth_users_username ON auth_users(username)")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_auth_perm_user_module ON auth_account_permissions(username, module_code)")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_auth_login_logs_time ON auth_login_logs(event_time)")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_security_users_username ON security_users(username)")
        for role_code, role_name, role_en in roles:
            cur.execute(
                """
                INSERT INTO security_roles(role_code, role_name, role_name_en, description, is_system_role, created_at, updated_at)
        ...
        for username, password, display_name, role_code in default_users:
            cur.execute(
                """
                INSERT INTO security_users(username, password_hash, display_name, is_active, created_at, updated_at)
                VALUES (%s,%s,%s,1,%s,%s)
                ON CONFLICT (username) DO NOTHING
                """,
                (username, _v255_pbkdf2_hash(password), display_name, now, now),
            )
            cur.execute(
                """
                INSERT INTO security_user_roles(username, role_code, created_at)
                VALUES (%s,%s,%s)
                ON CONFLICT (username, role_code) DO NOTHING
                """,
                (username, role_code, now),
            )
        for role_code, _rn, _re in roles:
            for module_code, module_no, module_name, module_en in modules:
                all_true = role_code == "admin"
                can_view = 1 if all_true or role_code in {"manager", "leader", "operator", "viewer", "auditor"} else 0
                cur.execute(
                    """
                    INSERT INTO security_module_permissions(role_code,module_code,module_no,module_name,module_name_en,can_view,can_create,can_edit,can_delete,can_import,can_export,can_backup,can_restore,can_manage,updated_at)
                    VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
                    ON CONFLICT (role_code, module_code) DO NOTHING
                    """,
                    (role_code, module_code, module_no, module_name, module_en,
                     1 if all_true else can_view, 1 if all_true else 0, 1 if all_true else 0, 1 if all_true else 0,
                     1 if all_true else 0, 1 if all_true else 0, 1 if all_true else 0, 1 if all_true else 0, 1 if all_true else 0, now),
                )
        cur.execute(
            """
            INSERT INTO auth_security_settings(setting_key, setting_value, note, updated_at)
            VALUES ('idle_timeout_minutes','15','Idle logout minutes',%s)
            ON CONFLICT (setting_key) DO NOTHING
            """,
            (now,),
        )
    conn.commit()
    _V255_PG_AUTH_SCHEMA_READY = True


def _v255_pg_prepare_for_sql(sql: str | None) -> None:
    if not is_postgres_enabled():
        return
    if _v255_is_auth_sql(sql):
        _v255_pg_ensure_auth_schema()
    else:
        ensure_database()
...
def execute(sql: str, params: Iterable[Any] | None = None) -> int:  # type: ignore[override]
    ...
    out = _v29_pg_retry(_v29_pg_write_once, sql, params or ())
    _v30_clear_cache_for_sql(sql)
    if (not _v255_is_auth_sql(sql)) and _should_after_write_sync(sql):
        try:
            mark_data_changed(reason="PostgreSQL data changed", source_sql=sql)
        except Exception:
            pass
...
def audit_v255_startup_login_hot_path() -> dict[str, Any]:
    return {
        "version": "V255_APP_STARTUP_LOGIN_HOT_PATH",
        "postgres_enabled": is_postgres_enabled(),
        "auth_schema_ready": bool(_V255_PG_AUTH_SCHEMA_READY),
        "auth_sql_skips_full_business_schema": True,
        "app_open_before_login_skips_database": True,
    }

# ===== END V255 APP STARTUP / LOGIN HOT PATH FOR POSTGRESQL =====
