...
# ======================= V166E DUPLICATE MASTER ROW GUARD =======================
try:
    _v166e_prev_save_work_orders = save_work_orders
except Exception:  # pragma: no cover
    _v166e_prev_save_work_orders = None
try:
    _v166e_prev_save_employees = save_employees
except Exception:  # pragma: no cover
    _v166e_prev_save_employees = None


def save_work_orders(df: pd.DataFrame) -> dict:  # type: ignore[override]
    from services.duplicate_guard_service import dedupe_work_orders_dataframe
    clean = dedupe_work_orders_dataframe(df)
    if _v166e_prev_save_work_orders is not None:
        result = _v166e_prev_save_work_orders(clean)
    else:
        result = {"saved": len(clean), "updated": len(clean), "inserted": 0, "deleted": 0, "skipped": 0}
    try:
        result = dict(result or {})
        result["v166e_duplicate_guard"] = True
        result["v166e_input_rows"] = int(len(df)) if isinstance(df, pd.DataFrame) else 0
        result["v166e_saved_rows_after_dedupe"] = int(len(clean))
        result["v166e_dropped_duplicates"] = max(0, result["v166e_input_rows"] - result["v166e_saved_rows_after_dedupe"])
    except Exception:
        pass
    return result
...
def save_employees(df: pd.DataFrame) -> dict:  # type: ignore[override]
    ...
    if _v166e_prev_save_employees is not None:
        result = _v166e_prev_save_employees(clean)
    else:
        result = {"saved": len(clean), "updated": len(clean), "inserted": 0, "deleted": 0, "skipped": 0}
    try:
        result = dict(result or {})
        ...
        pass
    return result
# ===================== END V166E DUPLICATE MASTER ROW GUARD =====================
