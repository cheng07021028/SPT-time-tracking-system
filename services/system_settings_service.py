...
# ================= V34 NEON FAST SYSTEM SETTINGS AUTHORITY｜2026-06-02 =================
# Purpose:
# - 13 System Settings must open in seconds.
# - Neon/PostgreSQL is the single source of truth when DATABASE_URL is configured.
# - No local permanent JSON/GitHub authority reads/writes on page entry or save path.
# - All schema checks are process-level one-time only.
try:
    _v34_prev_ensure_system_settings_schema = ensure_system_settings_schema  # type: ignore[name-defined]
    _v34_prev_load_process_categories_df = load_process_categories_df  # type: ignore[name-defined]
    _v34_prev_load_process_options_df = load_process_options_df  # type: ignore[name-defined]
    _v34_prev_load_rest_periods_df = load_rest_periods_df  # type: ignore[name-defined]
except Exception:
    _v34_prev_ensure_system_settings_schema = None
    _v34_prev_load_process_categories_df = None
    _v34_prev_load_process_options_df = None
    _v34_prev_load_rest_periods_df = None

_V34_SYS_SCHEMA_READY = False
_V34_SYS_CACHE: dict[str, tuple[float, pd.DataFrame]] = {}
_V34_SYS_CACHE_SECONDS = 20.0


def _v34_sys_pg_enabled() -> bool:
    try:
...
def _v34_bool_int(v, default: int = 1) -> int:
...
def _v34_int(v, default: int = 0) -> int:
    try:
        if pd.isna(v):
            return int(default)
    except Exception:
...
def _v34_text(v, default: str = "") -> str:
    try:
        if pd.isna(v):
            return default
    except Exception:
        pass
    text = str(v if v is not None else default).strip()
    return text


def _v34_sys_clear_cache() -> None:
    try:
        _V34_SYS_CACHE.clear()
    except Exception:
        pass


def _v34_sys_cached_df(key: str, loader) -> pd.DataFrame:
    try:
        import time
        now_s = time.time()
        got = _V34_SYS_CACHE.get(key)
        if got and (now_s - got[0]) <= _V34_SYS_CACHE_SECONDS:
            return got[1].copy()
        df = loader()
        if df is None:
            df = pd.DataFrame()
        _V34_SYS_CACHE[key] = (now_s, df.copy())
        return df.copy()
    except Exception:
        try:
            return loader()
...
def _v34_ensure_system_settings_neon_schema() -> None:
    global _V34_SYS_SCHEMA_READY
    if _V34_SYS_SCHEMA_READY:
        return
    if not _v34_sys_pg_enabled():
        if callable(_v34_prev_ensure_system_settings_schema):
            try:
                _v34_prev_ensure_system_settings_schema()
            except Exception:
                pass
        _V34_SYS_SCHEMA_READY = True
        return
    try:
        ...
        with _db._v25_pg_connect() as conn:  # type: ignore[attr-defined]
            with conn.cursor() as cur:
                stmts = [
                    ...
                    "ALTER TABLE process_options ADD COLUMN IF NOT EXISTS category_name TEXT",
                    "ALTER TABLE process_options ADD COLUMN IF NOT EXISTS deleted_at TEXT",
                    "ALTER TABLE process_options ADD COLUMN IF NOT EXISTS deleted_by TEXT",
                    "ALTER TABLE process_options ADD COLUMN IF NOT EXISTS delete_reason TEXT",
                    "ALTER TABLE process_options ADD COLUMN IF NOT EXISTS version INTEGER DEFAULT 1",
                    "ALTER TABLE rest_periods ADD COLUMN IF NOT EXISTS deleted_at TEXT",
                    "ALTER TABLE rest_periods ADD COLUMN IF NOT EXISTS deleted_by TEXT",
                    "ALTER TABLE rest_periods ADD COLUMN IF NOT EXISTS delete_reason TEXT",
                    "ALTER TABLE rest_periods ADD COLUMN IF NOT EXISTS version INTEGER DEFAULT 1",
                    "ALTER TABLE system_settings ADD COLUMN IF NOT EXISTS deleted_at TEXT",
                    "ALTER TABLE system_settings ADD COLUMN IF NOT EXISTS deleted_by TEXT",
                    "ALTER TABLE system_settings ADD COLUMN IF NOT EXISTS delete_reason TEXT",
                    "CREATE INDEX IF NOT EXISTS idx_v34_process_categories_fast ON process_categories(is_active, sort_order, id) WHERE COALESCE(deleted_at,'')=''",
                    "CREATE INDEX IF NOT EXISTS idx_v34_process_options_fast ON process_options(category_name, is_active, sort_order, id) WHERE COALESCE(deleted_at,'')=''",
                    "CREATE INDEX IF NOT EXISTS idx_v34_rest_periods_fast ON rest_periods(is_active, sort_order, id) WHERE COALESCE(deleted_at,'')=''",
                    "CREATE INDEX IF NOT EXISTS idx_v34_system_settings_key ON system_settings(setting_key) WHERE COALESCE(deleted_at,'')=''",
                ]
                for stmt in stmts:
                    try:
                        cur.execute(stmt)
                        ...
                        pass
                # Seed common category only if truly empty.
                try:
                    cur.execute("SELECT COUNT(*) AS c FROM process_categories WHERE COALESCE(deleted_at,'')=''")
                    row = cur.fetchone() or {}
                    if int(row.get("c") or 0) == 0:
                        now = _v34_sys_now()
                        cur.execute("INSERT INTO process_categories(category_name, is_active, sort_order, note, created_at, updated_at) VALUES (%s,1,0,%s,%s,%s)", ("全部 / 通用", "system default", now, now))
                except Exception:
                    pass
            conn.commit()
    except Exception:
...
def ensure_system_settings_schema() -> None:  # type: ignore[override]
    _v34_ensure_system_settings_neon_schema()


def restore_system_settings_from_permanent(force: bool = False) -> dict[str, Any]:  # type: ignore[override]
    if _v34_sys_pg_enabled():
        _v34_ensure_system_settings_neon_schema()
        return {"ok": True, "skipped": True, "authority": "Neon/PostgreSQL", "message": "Neon is authority; no local permanent restore on page entry."}
    if callable(globals().get("_v30017_prev_restore_system_settings_from_permanent")):
        try:
            return _v30017_prev_restore_system_settings_from_permanent(force=force)  # type: ignore[name-defined]
        except Exception as exc:
...
def export_system_settings_permanent(reason: str = "system_settings_changed", write_history: bool = True) -> dict[str, Any]:  # type: ignore[override]
    if _v34_sys_pg_enabled():
        _v34_ensure_system_settings_neon_schema()
        return {"ok": True, "skipped": True, "authority": "Neon/PostgreSQL", "reason": reason, "message": "Settings are saved in Neon; foreground JSON/GitHub export disabled."}
    if callable(globals().get("_v30017_prev_export_system_settings_permanent")):
        try:
            return _v30017_prev_export_system_settings_permanent(reason=reason, write_history=write_history)  # type: ignore[name-defined]
        except Exception as exc:
            return {"ok": False, "error": str(exc)[:300]}
    return {"ok": True, "skipped": True}
...
def load_process_categories_df(active_only: bool = False) -> pd.DataFrame:  # type: ignore[override]
    if not _v34_sys_pg_enabled():
        if callable(_v34_prev_load_process_categories_df):
            return _v34_prev_load_process_categories_df(active_only=active_only)
        return pd.DataFrame()
    _v34_ensure_system_settings_neon_schema()
    def _load():
        ...
        df = query_df(f"SELECT id, category_name, COALESCE(is_active,1) AS is_active, COALESCE(sort_order,0) AS sort_order, note, created_at, updated_at FROM process_categories {where} ORDER BY sort_order, id")
        if df is None or df.empty:
            return pd.DataFrame([{"id": "", "category_name": "全部 / 通用", "is_active": 1, "sort_order": 0, "note": "", "created_at": "", "updated_at": ""}])
        return df
    return _v34_sys_cached_df(f"categories:{active_only}", _load)


def load_process_options_df(active_only: bool = False) -> pd.DataFrame:  # type: ignore[override]
    if not _v34_sys_pg_enabled():
        if callable(_v34_prev_load_process_options_df):
            return _v34_prev_load_process_options_df(active_only=active_only)
        return pd.DataFrame()
    _v34_ensure_system_settings_neon_schema()
    def _load():
        from services.db_service import query_df
        ...
        if active_only:
            where += " AND COALESCE(is_active, active, 1)=1"
        return query_df(f"SELECT id, COALESCE(NULLIF(category_name,''),'全部 / 通用') AS category_name, process_name, COALESCE(is_active,active,1) AS is_active, COALESCE(sort_order,0) AS sort_order, note, created_at, updated_at FROM process_options {where} ORDER BY category_name, sort_order, id")
    return _v34_sys_cached_df(f"process:{active_only}", _load)


def load_rest_periods_df(active_only: bool = False) -> pd.DataFrame:  # type: ignore[override]
    if not _v34_sys_pg_enabled():
        if callable(_v34_prev_load_rest_periods_df):
            return _v34_prev_load_rest_periods_df(active_only=active_only)
        return pd.DataFrame()
    _v34_ensure_system_settings_neon_schema()
    def _load():
        from services.db_service import query_df
        ...
        if active_only:
            where += " AND COALESCE(is_active,active,1)=1"
        return query_df(f"SELECT id, name, start_time, end_time, COALESCE(is_active,active,1) AS is_active, COALESCE(sort_order,0) AS sort_order, created_at, updated_at FROM rest_periods {where} ORDER BY sort_order, id")
    return _v34_sys_cached_df(f"rest:{active_only}", _load)
...
def load_process_category_choices(include_common: bool = True) -> list[str]:  # type: ignore[override]
    ...
    try:
        ...
        if isinstance(df, pd.DataFrame) and not df.empty:
            cats.extend([_v34_text(x) for x in df.get("category_name", []).tolist() if _v34_text(x)])
    except Exception:
        pass
    try:
        dfp = load_process_options_df(active_only=False)
        if isinstance(dfp, pd.DataFrame) and not dfp.empty and "category_name" in dfp.columns:
            cats.extend([_v34_text(x) for x in dfp["category_name"].tolist() if _v34_text(x)])
    except Exception:
        pass
    if include_common:
        cats.insert(0, "全部 / 通用")
...
def _v34_setting_get(key: str, default: str = "") -> str:
    _v34_ensure_system_settings_neon_schema()
    try:
        from services.db_service import query_one
        row = query_one("SELECT setting_value FROM system_settings WHERE setting_key=? AND COALESCE(deleted_at,'')='' ORDER BY updated_at DESC NULLS LAST LIMIT 1", (key,)) or {}
        val = _v34_text(row.get("setting_value"), default)
        return val if val != "" else default
    except Exception:
        return default


def _v34_setting_set(key: str, value: str, note: str = "") -> None:
    _v34_ensure_system_settings_neon_schema()
    from services.db_service import execute
    now = _v34_sys_now()
    # Avoid relying on a unique constraint; delete old active rows then insert current value.
    execute("UPDATE system_settings SET deleted_at=?, deleted_by='system', delete_reason='superseded' WHERE setting_key=? AND COALESCE(deleted_at,'')=''", (now, key))
    execute("INSERT INTO system_settings(setting_key, setting_value, note, updated_at, updated_by, deleted_at) VALUES (?,?,?,?,?,'')", (key, str(value), str(note), now, "system"))
    _v34_sys_clear_cache()


def get_default_process_category() -> str:  # type: ignore[override]
    return _v34_setting_get("default_process_category", "全部 / 通用")


def save_default_process_category(category_name: str) -> str:  # type: ignore[override]
    val = _v34_text(category_name, "全部 / 通用") or "全部 / 通用"
    _v34_setting_set("default_process_category", val, "13 system default category")
    return val


def get_live_page_reset_time() -> str:  # type: ignore[override]
    return _v34_setting_get("live_page_reset_time", "02:00")


def save_live_page_reset_time(value: str) -> str:  # type: ignore[override]
    val = _v34_text(value, "02:00") or "02:00"
    _v34_setting_set("live_page_reset_time", val, "01 page daily display reset time")
    return val
...
def save_process_categories_df(df: pd.DataFrame) -> int:  # type: ignore[override]
    ...
    for _, row in (df if isinstance(df, pd.DataFrame) else pd.DataFrame()).iterrows():
        name = _v34_text(row.get("category_name") or row.get("類別") or row.get("Category"))
        if not name:
            continue
        rid = _v34_int(row.get("id"), 0)
        params_common = (name, _v34_bool_int(row.get("is_active"), 1), _v34_int(row.get("sort_order"), 0), _v34_text(row.get("note")), now)
        if rid > 0:
            execute("UPDATE process_categories SET category_name=?, is_active=?, sort_order=?, note=?, updated_at=?, version=COALESCE(version,1)+1 WHERE id=?", (*params_common, rid))
        else:
            execute("INSERT INTO process_categories(category_name, is_active, sort_order, note, created_at, updated_at, deleted_at) VALUES (?,?,?,?,?,?,'')", (*params_common, now))
        count += 1
    _v34_sys_clear_cache()
    return count


def delete_process_categories(ids: list[int]) -> int:  # type: ignore[override]
    _v34_ensure_system_settings_neon_schema()
    clean = [_v34_int(x, 0) for x in (ids or []) if _v34_int(x, 0) > 0]
    if not clean:
        return 0
    from services.db_service import query_df, execute
    ph = ",".join(["?"] * len(clean))
    df = query_df(f"SELECT category_name FROM process_categories WHERE id IN ({ph})", tuple(clean))
    names = [_v34_text(x) for x in (df.get("category_name", []).tolist() if isinstance(df, pd.DataFrame) and not df.empty else []) if _v34_text(x)]
    now = _v34_sys_now()
    execute(f"UPDATE process_categories SET deleted_at=?, deleted_by='admin', delete_reason='delete_from_13' WHERE id IN ({ph})", tuple([now] + clean))
    if names:
        ph2 = ",".join(["?"] * len(names))
        execute(f"UPDATE process_options SET deleted_at=?, deleted_by='admin', delete_reason='category_deleted' WHERE category_name IN ({ph2}) AND COALESCE(deleted_at,'')=''", tuple([now] + names))
    _v34_sys_clear_cache()
    return len(clean)
...
def save_process_options_df(df: pd.DataFrame) -> int:  # type: ignore[override]
    ...
    count = 0
    for _, row in (df if isinstance(df, pd.DataFrame) else pd.DataFrame()).iterrows():
        process = _v34_text(row.get("process_name") or row.get("工段名稱") or row.get("工段名稱 / Process"))
        if not process:
            continue
        category = _v34_text(row.get("category_name") or row.get("category") or row.get("類別") or row.get("類別 / Category"), "全部 / 通用") or "全部 / 通用"
        rid = _v34_int(row.get("id"), 0)
        common = (category, process, _v34_bool_int(row.get("is_active"), 1), _v34_int(row.get("sort_order"), 0), _v34_text(row.get("note") or row.get("備註")), now)
        if rid > 0:
            execute("UPDATE process_options SET category_name=?, process_name=?, is_active=?, active=?, sort_order=?, note=?, updated_at=?, version=COALESCE(version,1)+1 WHERE id=?", (category, process, common[2], common[2], common[3], common[4], now, rid))
        else:
            execute("INSERT INTO process_options(category_name, process_name, is_active, active, sort_order, note, created_at, updated_at, deleted_at) VALUES (?,?,?,?,?,?,?,?, '')", (category, process, common[2], common[2], common[3], common[4], now, now))
        count += 1
    _v34_sys_clear_cache()
    return count


def delete_process_options(ids: list[int]) -> int:  # type: ignore[override]
    clean = [_v34_int(x, 0) for x in (ids or []) if _v34_int(x, 0) > 0]
    if not clean:
        return 0
    _v34_ensure_system_settings_neon_schema()
    from services.db_service import execute
    ph = ",".join(["?"] * len(clean))
    execute(f"UPDATE process_options SET deleted_at=?, deleted_by='admin', delete_reason='delete_from_13' WHERE id IN ({ph})", tuple([_v34_sys_now()] + clean))
    _v34_sys_clear_cache()
    return len(clean)
...
def save_rest_periods_df(df: pd.DataFrame) -> int:  # type: ignore[override]
    ...
    count = 0
    for _, row in (df if isinstance(df, pd.DataFrame) else pd.DataFrame()).iterrows():
        name = _v34_text(row.get("name") or row.get("休息名稱"), "休息") or "休息"
        start = _v34_text(row.get("start_time") or row.get("開始時間"))
        end = _v34_text(row.get("end_time") or row.get("結束時間"))
        if not start or not end:
            continue
        rid = _v34_int(row.get("id"), 0)
        active = _v34_bool_int(row.get("is_active"), 1)
        sort_order = _v34_int(row.get("sort_order"), 0)
        if rid > 0:
            execute("UPDATE rest_periods SET name=?, start_time=?, end_time=?, is_active=?, active=?, sort_order=?, updated_at=?, version=COALESCE(version,1)+1 WHERE id=?", (name, start, end, active, active, sort_order, now, rid))
        else:
            execute("INSERT INTO rest_periods(name, start_time, end_time, is_active, active, sort_order, created_at, updated_at, deleted_at) VALUES (?,?,?,?,?,?,?,?, '')", (name, start, end, active, active, sort_order, now, now))
        count += 1
    _v34_sys_clear_cache()
    return count


def delete_rest_periods(ids: list[int]) -> int:  # type: ignore[override]
    clean = [_v34_int(x, 0) for x in (ids or []) if _v34_int(x, 0) > 0]
    if not clean:
        return 0
    _v34_ensure_system_settings_neon_schema()
    from services.db_service import execute
    ph = ",".join(["?"] * len(clean))
    execute(f"UPDATE rest_periods SET deleted_at=?, deleted_by='admin', delete_reason='delete_from_13' WHERE id IN ({ph})", tuple([_v34_sys_now()] + clean))
    _v34_sys_clear_cache()
    return len(clean)


def audit_v34_system_settings_neon_fastpath() -> dict[str, Any]:
